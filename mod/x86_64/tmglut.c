...
#define e_nmda _p[6]
#define e_nmda_columnindex 6
#define tau _p[7]
#define tau_columnindex 7
#define tauR _p[8]
#define tauR_columnindex 8
#define tauF _p[9]
#define tauF_columnindex 9
#define U _p[10]
#define U_columnindex 10
#define u0 _p[11]
#define u0_columnindex 11
#define ca_ratio_ampa _p[12]
#define ca_ratio_ampa_columnindex 12
#define ca_ratio_nmda _p[13]
#define ca_ratio_nmda_columnindex 13
#define mg _p[14]
#define mg_columnindex 14
#define q _p[15]
#define q_columnindex 15
#define base _p[16]
#define base_columnindex 16
#define f_ampa _p[17]
#define f_ampa_columnindex 17
#define f_nmda _p[18]
#define f_nmda_columnindex 18
#define i _p[19]
...
#define ica_ampa _p[30]
#define ica_ampa_columnindex 30
#define ica_nmda _p[31]
#define ica_nmda_columnindex 31
#define factor_ampa _p[32]
#define factor_ampa_columnindex 32
#define factor_nmda _p[33]
#define factor_nmda_columnindex 33
#define x _p[34]
#define x_columnindex 34
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 5;
 	...
 	ivoc_help("help ?1 tmGlut /home/shadeform/Spillover_2/mod/tmglut.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; NrnThread* _nt;
 ...
 _args[2] = _args[2] * exp ( - ( t - _args[4] ) / tauR ) ;
   ...
   _lweight_nmda = _lweight_ampa * nmda_ratio ;
    ...
    double __primary = (A_ampa + _lweight_ampa * factor_ampa * _args[3]) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( ( - 1.0 )*( q ) ) / tau1_ampa ) ) )*( - ( 0.0 ) / ( ( ( - 1.0 )*( q ) ) / tau1_ampa ) - __primary );
    A_ampa += __primary;
  } else {
 A_ampa = A_ampa + _lweight_ampa * factor_ampa * _args[3] ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B_ampa;
    double __primary = (B_ampa + _lweight_ampa * factor_ampa * _args[3]) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( ( - 1.0 )*( q ) ) / tau2_ampa ) ) )*( - ( 0.0 ) / ( ( ( - 1.0 )*( q ) ) / tau2_ampa ) - __primary );
    B_ampa += __primary;
  } else {
 B_ampa = B_ampa + _lweight_ampa * factor_ampa * _args[3] ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = A_nmda;
    double __primary = (A_nmda + _lweight_nmda * factor_nmda * _args[3]) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( ( - 1.0 )*( q ) ) / tau1_nmda ) ) )*( - ( 0.0 ) / ( ( ( - 1.0 )*( q ) ) / tau1_nmda ) - __primary );
    A_nmda += __primary;
  } else {
 A_nmda = A_nmda + _lweight_nmda * factor_nmda * _args[3] ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B_nmda;
    double __primary = (B_nmda + _lweight_nmda * factor_nmda * _args[3]) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( ( - 1.0 )*( q ) ) / tau2_nmda ) ) )*( - ( 0.0 ) / ( ( ( - 1.0 )*( q ) ) / tau2_nmda ) - __primary );
    B_nmda += __primary;
  } else {
 B_nmda = B_nmda + _lweight_nmda * factor_nmda * _args[3] ;
     }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
 ...
 _lmggate = 1.0 / ( 1.0 + exp ( - 0.062 * v ) * ( mg / 3.57 ) ) ;
   ...
   ica_ampa = ca_ratio_ampa * _litotal ;
   i_ampa = _litotal - ica_ampa ;
   g_nmda = B_nmda - A_nmda ;
   _litotal = g_nmda * ( v - e_nmda ) * _lmggate ;
   ica_nmda = ca_ratio_nmda * _litotal ;
   i_nmda = _litotal - ica_nmda ;
   ica_nmda = ica_ampa + ica_nmda ;
   i = i_ampa + i_nmda ;
   g = g_ampa + g_nmda ;
   }
 _current += i;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/shadeform/Spillover_2/mod/tmglut.mod";
static const char* nmodl_file_text = 
  ...
  "    ica_ampa = ca_ratio_ampa*itotal\n"
  "    i_ampa = itotal - ica_ampa\n"
  "    \n"
  "    g_nmda = B_nmda - A_nmda\n"
  "    itotal = g_nmda*(v - e_nmda)*mggate\n"
  "    ica_nmda = ca_ratio_nmda*itotal\n"
  "    i_nmda = itotal - ica_nmda\n"
  "    ica_nmda = ica_ampa + ica_nmda\n"
  "    i = i_ampa + i_nmda\n"
  "    g = g_ampa + g_nmda\n"
  "}\n"

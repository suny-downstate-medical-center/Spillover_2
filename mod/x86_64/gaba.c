...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 1, "pntproc");
 	...
 	ivoc_help("help ?1 gaba /www/projects/Spillover/mod/gaba.mod\n");
...
double g ( _threadargsprotocomma_ double _lx ) {
 ...
 if ( _lx < onset ) {
     ...
     _la = 1.0 - ( _lx - onset ) / tau ;
     _le = exp ( _la ) ;
     _lg = ( _lx - onset ) / tau * _le ;
     }
   
return _lg;
 }
 
static double _hoc_g(void* _vptr) {
 double _r;
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   i = gmax * g ( _threadargscomma_ t ) * ( v - Erev ) ;
   }
 _current += i;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/www/projects/Spillover/mod/gaba.mod";
static const char* nmodl_file_text = 

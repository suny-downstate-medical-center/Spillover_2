...
#define A _p[19]
#define A_columnindex 19
#define B _p[20]
#define B_columnindex 20
#define C _p[21]
#define C_columnindex 21
#define D _p[22]
#define D_columnindex 22
#define factor_nmda _p[23]
#define factor_nmda_columnindex 23
#define factor_ampa _p[24]
#define factor_ampa_columnindex 24
#define block _p[25]
#define block_columnindex 25
#define DA _p[26]
...
#define MgBlock MgBlock_glutamate
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 glutamate /home/shadeform/Spillover_2/mod/glutamate.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; NrnThread* _nt;
 ...
 _tsav = t; {
    ...
    double __state = A;
    double __primary = (A + _args[0] * factor_nmda * ratio) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( ( - 1.0 ) / tau1_nmda )*( q ) ) ) )*( - ( 0.0 ) / ( ( ( - 1.0 ) / tau1_nmda )*( q ) ) - __primary );
    A += __primary;
  } else {
 A = A + _args[0] * factor_nmda * ratio ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B;
    double __primary = (B + _args[0] * factor_nmda * ratio) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( ( - 1.0 ) / tau2_nmda )*( q ) ) ) )*( - ( 0.0 ) / ( ( ( - 1.0 ) / tau2_nmda )*( q ) ) - __primary );
    B += __primary;
  } else {
 B = B + _args[0] * factor_nmda * ratio ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = C;
    double __primary = (C + _args[0] * factor_ampa) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau1_ampa ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau1_ampa ) - __primary );
    C += __primary;
  } else {
 C = C + _args[0] * factor_ampa ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = D;
    double __primary = (D + _args[0] * factor_ampa) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau2_ampa ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau2_ampa ) - __primary );
    D += __primary;
  } else {
 D = D + _args[0] * factor_ampa ;
     }
 } }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/shadeform/Spillover_2/mod/glutamate.mod";
static const char* nmodl_file_text = 
  "COMMENT\n"

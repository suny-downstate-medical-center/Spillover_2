...
#define nrn_init _nrn_init__catdyn
#define _nrn_initial _nrn_initial__catdyn
#define nrn_cur _nrn_cur__catdyn
#define _nrn_current _nrn_current__catdyn
#define nrn_jacob _nrn_jacob__catdyn
#define nrn_state _nrn_state__catdyn
#define _net_receive _net_receive__catdyn 
#define state state__catdyn 
...
#define cati _p[5]
#define cati_columnindex 5
#define Dcati _p[6]
#define Dcati_columnindex 6
#define icat _p[7]
#define icat_columnindex 7
...
#define _ion_cati	*_ppvar[1]._pval
#define _style_cat	*((int*)_ppvar[2]._pvoid)
...
static void nrn_alloc(Prop* _prop) {
 ...
 nrn_promote(prop_ion, 3, 0);
 	...
 	_ppvar[1]._pval = &prop_ion->param[1]; /* cati */
 	_ppvar[2]._pvoid = (void*)(&(prop_ion->dparam[0]._i)); /* iontype for cat */
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 catdyn /home/shadeform/Spillover_2/mod/catdyn.mod\n");
...
static int _reset;
static char *modelname = "Calcium dynamics for T calcium pool";
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {int _reset = 0; {
   drive_channel = - drive * icat / ( 2.0 * FARADAY * depth ) ;
   if ( drive_channel <= 0. ) {
     drive_channel = 0. ;
     }
   drive_pump = - kt * cati / ( cati + kd ) ;
   Dcati = ( drive_channel + pump * drive_pump + ( cainf - cati ) / taur ) ;
   }
 return _reset;
}
 ...
 static int state (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
   ...
   drive_pump = - kt * cati / ( cati + kd ) ;
    cati = cati + (1. - exp(dt*(( ( ( ( - 1.0 ) ) ) / taur ))))*(- ( ( drive_channel + ( pump )*( drive_pump ) + ( ( cainf ) ) / taur ) ) / ( ( ( ( ( - 1.0 ) ) ) / taur ) ) - cati) ;
   }
  return 0;
}
...
static void _ode_matsol(NrnThread* _nt, _Memb_list* _ml, int _type) {
 ...
 static void _update_ion_pointer(Datum* _ppvar) {
   ...
   nrn_update_ion_pointer(_cat_sym, _ppvar, 1, 1);
 }
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = cati_columnindex;  _dlist1[0] = Dcati_columnindex;
_first = 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/shadeform/Spillover_2/mod/catdyn.mod";
static const char* nmodl_file_text = 

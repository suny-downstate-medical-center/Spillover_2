...
#define kb _p[4]
#define kb_columnindex 4
#define kt _p[5]
#define kt_columnindex 5
#define kd _p[6]
#define kd_columnindex 6
#define pump _p[7]
#define pump_columnindex 7
#define ca_nmdai _p[8]
#define ca_nmdai_columnindex 8
#define Dca_nmdai _p[9]
#define Dca_nmdai_columnindex 9
#define ica_nmda _p[10]
#define ica_nmda_columnindex 10
#define drive_channel _p[11]
#define drive_channel_columnindex 11
#define drive_pump _p[12]
#define drive_pump_columnindex 12
#define v _p[13]
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 cadyn_nmda /home/shadeform/Spillover_2/mod/cadyn_nmda.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {int _reset = 0; {
   drive_channel = - drive * ica_nmda / ( 2.0 * FARADAY * depth * ( 1.0 + kb ) ) ;
   if ( drive_channel <= 0. ) {
     drive_channel = 0. ;
     }
   drive_pump = - kt * ca_nmdai / ( ca_nmdai + kd ) ;
   Dca_nmdai = ( drive_channel + pump * drive_pump + ( cainf - ca_nmdai ) / taur ) ;
   }
 return _reset;
}
 ...
 drive_pump = - kt * ca_nmdai / ( ca_nmdai + kd ) ;
 Dca_nmdai = Dca_nmdai  / (1. - dt*( ( ( ( ( - 1.0 ) ) ) / taur ) )) ;
  return 0;
}
 /*END CVODE*/
 static int state (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
   ...
   drive_pump = - kt * ca_nmdai / ( ca_nmdai + kd ) ;
    ca_nmdai = ca_nmdai + (1. - exp(dt*(( ( ( ( - 1.0 ) ) ) / taur ))))*(- ( ( drive_channel + ( pump )*( drive_pump ) + ( ( cainf ) ) / taur ) ) / ( ( ( ( ( - 1.0 ) ) ) / taur ) ) - ca_nmdai) ;
   }
  return 0;
}
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = ca_nmdai_columnindex;  _dlist1[0] = Dca_nmdai_columnindex;
_first = 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/shadeform/Spillover_2/mod/cadyn_nmda.mod";
static const char* nmodl_file_text = 
  ...
  "    RANGE pump, cainf, taur, drive, kt, kd, kb, depth\n"
  "}\n"
  "\n"
  "UNITS {\n"
  "    (molar) = (1/liter) \n"
  ...
  "\n"
  "DERIVATIVE state { \n"
  "    drive_channel = -drive*ica_nmda/(2*FARADAY*depth*(1+kb))\n"
  "    if (drive_channel <= 0.) { drive_channel = 0. }\n"
  "    drive_pump = -kt*ca_nmdai/(ca_nmdai+kd)\n"
  "    ca_nmdai' = (drive_channel+pump*drive_pump+(cainf-ca_nmdai)/taur)\n"
  "}\n"
  "\n"
  "COMMENT\n"
  "\n"

...
class nmda_sim:
    ...
    def __init__(self, neuron_id,neuron_name,archive,length,cell_type_val):
        ...
        print(cell.max_dist())
        parent_sec_map, child_sec_map, sibling_sec_map = cell.get_sec_maps()
        print (" sibling_sec_map ", sibling_sec_map)
        ...
        for sec in h.allsec():
            ...
            dend_num = 0
            try:
                dend_num = int(sec_name[sec_name.find("[") + 1:sec_name.rfind("]")])
            except:
                pass
            print(dend_num)
            seg_list = []
            ...
            for node in param.nodes:
              node.value = 100
              if node.segment in seg_list:
                  node.value = 500            
            
            ps = h.PlotShape(False)
            ps.variable(param)
            ps.scale(0, 500)
            my_plot = ps.plot(plotly, color="black")
            if not os.path.isdir("plots/" + cell_type_val + "/" + archive.lower()):
                os.mkdir("plots/" + cell_type_val + "/" + archive.lower())
            if not os.path.isdir("plots/" + cell_type_val + "/" + archive.lower() + "/" + neuron_name):
                os.mkdir("plots/" + cell_type_val + "/" + archive.lower() + "/" + neuron_name)                
            input_html_file = "plots/" + cell_type_val + "/" + archive.lower() + "/" + neuron_name + "/" + archive.lower() + "_" + neuron_name + "_dend_" + str(dend_num) + ".html"
            my_plot.write_html(input_html_file)            
            ...
            #     sibling_index = sibling_name.find("[")
            #     sibling_num = int(sibling_name[sibling_index+1:sibling_name.find("]")])
            #     print ( " sibling num ", sibling_num ) 
            #     dend_record_list.append(sibling_num)
            
            dend_stim_list = []#[3,4,9,10,35,36]                    
            plateau_cluster_list = [sec_num]        
    
            # plateau_cluster_size = np.arange(25,31,1)
            ...
            ax_vs.set_ylabel('Vs (mV)'); 
            ax_vs.set_xlabel('t (ms)')

            ax_vspine = fig_vspine.add_subplot(111);
            ax_vspine.set_ylabel('Vspine (mV)'); 
            ax_vspine.set_xlabel('t (ms)')
            
            ax_vd = fig_vd.add_subplot(111); 
            ax_vd.set_ylabel('Vd (mV)'); 
            ax_vd.set_xlabel('t (ms)')
            colors = sns.color_palette("coolwarm", plateau_cluster_size.max())
            
            add_spine = 0
            on_spine = 1
            # insert_spines(self, section_list, start_pos, end_pos, num_spines = 20)
            ...
            for num_syns in plateau_cluster_size:
                ex = pe.Spillover_Experiment('record_ca', cell,input_dends=[sec_num])
            ...
            plt.savefig("plots/" + cell_type_val + "/" + archive.lower() + "/" + neuron_name + "/" + archive.lower() + "_" + neuron_name + "_dend_" + str(dend_num) + ".png")
